...
let S={team:[],streak:0,chem:0,momentum:0,wind:false,windAwarded:false,challengeSeen:false,specialSingles:false,tagBackup:null,exhibition:false,quickType:null,quickPlayer:null};
...
function resetClassicState(){clearStoryTimer();M=null;S={team:[],streak:0,chem:0,momentum:0,wind:false,windAwarded:false,challengeSeen:false,specialSingles:false,tagBackup:null,exhibition:false,quickType:null,quickPlayer:null,venue:null,attendance:null};}
...
function quickMatchMenu(){
 ...
 render(`<section class="framework-screen quick-framework">${shellBack()}<div class="framework-art"><img src="${wrestlerPng(w)}" alt="${w.name}"></div><div class="framework-copy"><div class="tv-kicker">EXHIBITION</div><h1>QUICK MATCH</h1><p>Create a one-on-one dream match using any wrestler from the Founding Twenty.</p><div class="framework-options"><button onclick="beginQuickSingles()"><b>SINGLES MATCH</b><small>Choose one wrestler and one opponent.</small><em>PLAYABLE</em></button><button disabled><b>TAG TEAM MATCH</b><small>Coming after Singles is confirmed stable.</small><em>NEXT PATCH</em></button></div></div></section>`)
}
function beginQuickSingles(){
 clearStoryTimer();M=null;overlay.innerHTML='';
 S={team:[],streak:0,chem:0,momentum:0,wind:false,windAwarded:false,challengeSeen:false,specialSingles:false,tagBackup:null,exhibition:true,quickType:'singles',quickPlayer:null,venue:null,attendance:null};
...
function match(){
 tvSting((S.exhibition||S.specialSingles)?'FEATURED SINGLES MATCH':'TAG TEAM GAUNTLET',(S.exhibition||S.specialSingles)?'ONE-ON-ONE':'THE BELL IS NEXT',`${currentVenue()} · ${attendance()} fans`);
 ...
 addBroadcast('broadcast',`${(S.exhibition||S.specialSingles)?'YOUR WRESTLER':'YOUR TEAM'}: ${S.team.map(wrestlerIntro).join(' / ')}`); addBroadcast('broadcast',`${(S.exhibition||S.specialSingles)?'OPPONENT':'OPPOSITION'}: ${S.opp.map(wrestlerIntro).join(' / ')}`);
...
function renderMatch(){
 ...
 <div class="broadcast-layout"><div class="broadcast-card">${card(p,'',true)}<small>${(S.exhibition||S.specialSingles)?'YOUR WRESTLER':'LEGAL WRESTLER'}</small></div><div id="broadcastFeed" class="broadcast-feed">${M.log.slice(-10).map((e,i)=>`<div class="broadcast-line ${e.type} ${i===M.log.slice(-10).length-1?'latest':''}">${e.type==='phase'?'<b>'+e.text+'</b>':e.text}</div>`).join('')}</div><div class="broadcast-card">${card(o,'',true)}<small>${(S.exhibition||S.specialSingles)?'OPPONENT':'LEGAL WRESTLER'}</small></div></div>
...
function resolveFinish(){
 ...
 M.ended=true;M.winner=winner;M.loser=loser;M.mvp=selectMVP(winnerTeam,winner);if(win&&!S.exhibition)S.streak++;
