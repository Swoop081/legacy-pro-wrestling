...
let S={team:[],streak:0,chem:0,momentum:0,wind:false,windAwarded:false,challengeSeen:false,specialSingles:false,tagBackup:null,mode:'classic',quickType:null,selection:[]};
...
function resetClassicState(){clearStoryTimer();M=null;S={team:[],streak:0,chem:0,momentum:0,wind:false,windAwarded:false,challengeSeen:false,specialSingles:false,tagBackup:null,venue:null,attendance:null,mode:'classic',quickType:null,selection:[]};}
...
function collectionProfile(id){
 ...
 render(`<section class="profile-screen"><div class="profile-nav"><button class="shell-back" onclick="collection()">← COLLECTION</button><button class="profile-play" onclick="collectionPlayNow('${w.id}')">PLAY NOW</button></div><div class="profile-art"><img src="${wrestlerPng(w)}" alt="${w.name}"></div><div class="profile-copy"><div class="profile-status">FOUNDING TWENTY · AVAILABLE</div><small>${w.title}</small><h1>${w.name}</h1><div class="profile-signature"><span>SIGNATURE MOVE</span><b>${w.signature}</b></div><p>${BIOS[w.id]||`${w.name} is a ${w.faction} competitor in Tag Team Gauntlet.`}</p><div class="profile-facts"><div><small>FACTION</small><b>${w.faction}</b></div><div><small>STYLE</small><b>${(typeof profileFor==='function'?profileFor(w).archetype:'Wrestler')}</b></div><div><small>OVERALL</small><b>${w.overall}</b></div><div><small>RARITY</small><b>${w.rarity}</b></div></div></div></section>`)
}
function quickMatchMenu(){
 clearStoryTimer();M=null;S.mode='exhibition';S.quickType=null;S.selection=[];S.team=[];S.opp=[];S.streak=0;
 const w=one(WRESTLERS);
 render(`<section class="framework-screen quick-framework">${shellBack()}<div class="framework-art"><img src="${wrestlerPng(w)}" alt="${w.name}"></div><div class="framework-copy"><div class="tv-kicker">EXHIBITION</div><h1>QUICK MATCH</h1><p>Select any member of the Founding Twenty and create a dream match with no gauntlet consequences.</p><div class="framework-options"><button onclick="beginQuickMatch('singles')"><b>SINGLES MATCH</b><small>Choose one wrestler and one opponent.</small><em>PLAY</em></button><button onclick="beginQuickMatch('tag')"><b>TAG TEAM MATCH</b><small>Choose all four competitors.</small><em>PLAY</em></button></div></div></section>`)
}
function collectionPlayNow(id){
 const w=WRESTLERS.find(x=>x.id===id);if(!w)return collection();
 S.mode='exhibition';S.quickType='singles';S.selection=[w];S.team=[];S.opp=[];S.streak=0;quickSelectScreen();
}
function beginQuickMatch(type){
 S.mode='exhibition';S.quickType=type;S.selection=[];S.team=[];S.opp=[];S.streak=0;quickSelectScreen();
}
function quickSlots(){return S.quickType==='tag'?['YOUR WRESTLER 1','YOUR WRESTLER 2','OPPONENT 1','OPPONENT 2']:['YOUR WRESTLER','OPPONENT']}
function quickSelectScreen(){
 const slots=quickSlots(),next=S.selection.length,complete=next===slots.length;
 render(`<section class="quick-select-screen"><div class="quick-select-top"><button class="shell-back" onclick="quickMatchMenu()">← QUICK MATCH</button><div><div class="tv-kicker">${S.quickType==='tag'?'TAG TEAM':'SINGLES'} EXHIBITION</div><h1>SELECT COMPETITORS</h1></div><button class="profile-play" ${complete?'':'disabled'} onclick="confirmQuickMatch()">CONTINUE</button></div><div class="selection-slots">${slots.map((label,i)=>{const w=S.selection[i];return `<article class="selection-slot ${i===next&&!complete?'active':''}"><small>${label}</small>${w?`<img src="${wrestlerPng(w)}" alt="${w.name}"><b>${w.name}</b><button onclick="clearQuickSelection(${i})">CHANGE</button>`:`<span>${i===next?'SELECT NOW':'WAITING'}</span>`}</article>`}).join('')}</div><div class="quick-roster">${WRESTLERS.map(w=>{const used=S.selection.some(x=>x.id===w.id);return `<button class="quick-roster-card ${used?'selected':''}" ${used||complete?'disabled':''} onclick="selectQuickWrestler('${w.id}')"><img src="${wrestlerPng(w)}" alt="${w.name}"><span><small>${w.title}</small><b>${w.name}</b></span></button>`}).join('')}</div></section>`)
}
function selectQuickWrestler(id){const w=WRESTLERS.find(x=>x.id===id);if(!w||S.selection.some(x=>x.id===id)||S.selection.length>=quickSlots().length)return;S.selection.push(w);quickSelectScreen()}
function clearQuickSelection(index){S.selection=S.selection.slice(0,index);quickSelectScreen()}
function confirmQuickMatch(){
 const needed=S.quickType==='tag'?4:2;if(S.selection.length!==needed)return;
 S.team=S.quickType==='tag'?S.selection.slice(0,2):[S.selection[0]];S.opp=S.quickType==='tag'?S.selection.slice(2,4):[S.selection[1]];S.specialSingles=S.quickType==='singles';S.venue=one(VENUES);S.attendance=Math.floor(rnd(11800,19800));quickMatchCard();
}
function quickMatchCard(){
 const singles=S.quickType==='singles',left=teamName(S.team),right=teamName(S.opp);
 render(`<section class="panel television-card"><div class="actions top-actions"><button class="btn secondary" onclick="quickSelectScreen()">EDIT MATCH</button><button class="btn broadcast-button" onclick="match()">BEGIN BROADCAST</button></div><div class="tv-kicker">QUICK MATCH · ${singles?'SINGLES':'TAG TEAM'}</div><h1 class="match-card-title">DREAM MATCH</h1><div class="cinematic-versus ${singles?'singles-cinematic':''}"><div class="hero-team hero-left">${S.team.map(w=>heroPortrait(w,'left')).join('')}<h2>${left}</h2></div><div class="giant-vs">VS</div><div class="hero-team hero-right">${S.opp.map(w=>heroPortrait(w,'right')).join('')}<h2>${right}</h2></div></div><div class="event-details"><span>🏟 ${currentVenue()}</span><span>👥 ${attendance()} · SOLD OUT</span><span>⚡ EXHIBITION</span></div></section>`)
}
...
function opponent(){
 ...
 render(`<section class="panel television-card"><div class="actions top-actions"><button class="btn broadcast-button" onclick="match()">BEGIN BROADCAST</button></div><div class="tv-kicker">${label}</div><h1 class="match-card-title">COMING UP NEXT</h1><div class="cinematic-versus"><div class="hero-team hero-left">${S.team.map(w=>heroPortrait(w,'left')).join('')}<h2>${left}</h2></div><div class="giant-vs">VS</div><div class="hero-team hero-right">${S.opp.map(w=>heroPortrait(w,'right')).join('')}<h2>${right}</h2></div></div><div class="event-details"><span>🏟 ${currentVenue()}</span><span>👥 ${attendance()} · SOLD OUT</span><span>🔥 STREAK ${S.streak}</span></div></section>`)
}
...
function match(){
 tvSting(S.specialSingles?'FEATURED SINGLES MATCH':(S.mode==='exhibition'?'QUICK MATCH':'TAG TEAM GAUNTLET'),S.specialSingles?'ONE-ON-ONE':'THE BELL IS NEXT',`${currentVenue()} · ${attendance()} fans`);
 clearStoryTimer();const q=S.mode==='classic'?walkout():null;if(q)return lose(`${q.name} walks away before the bell!`);
...
function resolveFinish(){
 ...
 M.ended=true;M.winner=winner;M.loser=loser;M.mvp=selectMVP(winnerTeam,winner);if(win&&S.mode==='classic')S.streak++;
