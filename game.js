...
function home(){clearStoryTimer();M=null;S={team:[],streak:0,chem:0,momentum:0,wind:false,windAwarded:false,challengeSeen:false,specialSingles:false,tagBackup:null};render(`<section class="panel home"><h1>TAG TEAM <span>GAUNTLET</span></h1><p>Build a team and survive the Gauntlet. Matches now play as televised wrestling stories, pausing only when your decision can change the outcome.</p><button class="btn" onclick="start()">START GAUNTLET</button></section>`)}
...
function discover(next){let r=S.team.length>1?rel(...S.team):null;if(r&&r.type==='legendary'){overlay.innerHTML=`<div class="overlay unlock-overlay"><div class="discover unlock-discover"><p>LEGENDARY TEAM DISCOVERED</p><div class="pair unlock-pair">${card(S.team[0])}${card(S.team[1])}</div><h1>${r.teamName}</h1><button class="btn" id="continue">CONTINUE</button></div></div>`;document.getElementById('continue').onclick=()=>{overlay.innerHTML='';next()}}else next()}
...
function renderMatch(){
 ...
 const feed=document.getElementById('broadcastFeed');if(feed)feed.scrollTop=feed.scrollHeight;if(M.waiting){const d=document.querySelector('.story-decision');if(d)setTimeout(()=>d.scrollIntoView({behavior:'smooth',block:'start'}),30)}
}
...
function acceptSinglesChallenge(index){
 ...
 render(`<section class="panel singles-intro"><small>FEATURED SINGLES MATCH</small><h1>${chosen.name}</h1><div class="singles-vs">VS</div><h1>${challenger.name}</h1><p>${challenger.title} issued the challenge. ${chosen.title} accepted it.</p><button class="btn" onclick="match()">BEGIN SINGLES MATCH</button></section>`);
}
