...
const MANAGERS=[
 {id:'vera-vale',name:'Vera Vale',title:'The Mastermind',initials:'VV',passive:2,ability:'Preparation',description:'Adds +2 to your team score every match.'},
 {id:'max-monroe',name:'Max Monroe',title:'The Mouthpiece',initials:'MM',passive:1,ability:'Crowd Control',description:'Adds +1 to your team score every match.'},
 {id:'silas-vane',name:'Silas Vane',title:'The Fixer',initials:'SV',passive:3,ability:'Calculated Risk',description:'Adds +3 to your team score every match.'},
 {id:'ruby-rhodes',name:'Ruby Rhodes',title:'The Motivator',initials:'RR',passive:2,ability:'Second Gear',description:'Adds +2 team score and strengthens positive interview outcomes.'}
];
function managerCard(m,action=''){return `<button class="manager-card" ${action?`onclick="${action}"`:''}><span class="manager-portrait">${m.initials}</span><small>${m.title}</small><h3>${m.name}</h3><b>${m.ability}</b><p>${m.description}</p></button>`}
function managerBonus(){return S.manager?.passive||0}
function managerStrip(){return S.manager?`<div class="active-manager"><span>${S.manager.initials}</span><div><small>AT RINGSIDE</small><b>${S.manager.name}</b><em>${S.manager.ability}</em></div></div>`:''}
...
function team(){clearStoryTimer();render(`<section class="panel"><h1 class="title">Your Team</h1><p class="sub">${rel(...S.team)?.teamName||S.team.map(x=>x.name).join(' & ')}</p>${managerStrip()}${S.nextMatchBonus?`<div class="next-match-boost">NEXT MATCH BONUS <b>+${S.nextMatchBonus}</b></div>`:''}<div class="cards two">${S.team.map(x=>card(x)).join('')}</div><div class="actions team-actions"><button class="btn" onclick="opponent()">FIND OPPONENT</button></div></section>`)}
function opponent(){
...
function renderMatch(){
 ...
 <div class="broadcast-top"><div><small>MATCH BROADCAST</small><h1>${M.phaseLabel}</h1></div><div class="story-chip">${M.story.name}</div></div>${managerStrip()}
...
function showChoiceEvent(event){overlay.innerHTML=`<div class="overlay event-overlay"><section class="between-event"><div class="tv-kicker">${event.eyebrow}</div><h1>${event.title}</h1><p>${event.copy}</p><div class="choice-grid event-choice-grid">${event.choices.map((c,i)=>`<button class="choice" onclick="resolveChoiceEvent(${i})"><b>${c[0]}</b><small>${c[1]}</small></button>`).join('')}</div></section></div>`;S.currentBetweenEvent=event}
...
function showManagerRecruitment(){const offers=pick(MANAGERS.filter(m=>m.id!==S.manager?.id),2);S.managerOffers=offers;overlay.innerHTML=`<div class="overlay event-overlay"><section class="between-event manager-recruit"><div class="tv-kicker">RARE RANDOM EVENT</div><h1>A Manager Wants In</h1><p>${S.manager?'A new representative offers to replace your current manager.':'Two representatives are interested in guiding your team for the rest of this Gauntlet run.'}</p><div class="manager-grid">${offers.map((m,i)=>managerCard(m,`recruitManager(${i})`)).join('')}</div><button class="btn secondary" onclick="finishBetweenEvent()">DECLINE</button></section></div>`}
