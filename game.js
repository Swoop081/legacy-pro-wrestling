...
function decisionHTML(){
 if(M.decisionOutcome){const x=M.decisionOutcome,sign=n=>n>0?`+${n}`:`${n}`;return `<div class="story-decision decision-outcome outcome-${x.key}"><small>YOUR CALL</small><h2>${x.label}</h2><p>${x.summary}</p><div class="outcome-deltas"><span><b>${sign(x.score)}</b><small>MATCH SCORE</small></span><span><b>${sign(x.control)}</b><small>CONTROL</small></span><span><b>${sign(x.crowd)}</b><small>CROWD</small></span></div><div class="outcome-progress">LIVE MATCH UPDATED</div><button class="btn outcome-continue" onclick="continueDecisionOutcome()">CONTINUE</button></div>`}
...
decisionHTML=function(){
 if(M.decisionOutcome){
  ...
  return `<div class="story-decision psychology-v2-foundation decision-outcome outcome-${x.key}"><div class="your-call-label">YOUR CALL</div><h2>${x.label}</h2><p>${x.summary}</p><div class="outcome-deltas"><span><b>${sign(x.score)}</b><small>MATCH SCORE</small></span><span><b>${sign(x.control)}</b><small>CONTROL</small></span><span><b>${sign(x.crowd)}</b><small>CROWD</small></span></div><div class="outcome-progress">LIVE MATCH UPDATED</div><button type="button" class="btn outcome-continue" onclick="continueDecisionOutcome()">CONTINUE</button></div>`;
 }
