...
function gauntletLiveFeudOrigin(){const c=liveLoad(),r=liveFeudOpponent(c),p=liveFounder(c.active),origins=[
 ...
 {title:'A BACKSTAGE ATTACK',npc:'leon-ward',copy:`${r.name} blindsides ${p.name} in the corridor before Leon Ward and security can separate them.`},
 ...
 ];const o=one(origins);c.world.feud.reason=o.copy;c.world.feudOriginSeen=c.month;liveSave(c);render(`<section class="panel lpw-feud-origin"><div class="tv-kicker">NEW MONTH · FEUD ORIGIN</div><h1>${o.title}</h1><div class="origin-matchup">${imageWithFallback(p,'portrait','art-portrait','matchPortrait')}<strong>VS</strong>${imageWithFallback(r,'portrait','art-portrait','matchPortrait')}</div><div class="live-npc-scene expanded">${npcImage(o.npc,'full')}<div><small>${npc(o.npc)?.role||''}</small><b>${npc(o.npc)?.name||''}</b><p>${o.copy}</p></div></div><p class="origin-close">The rivalry will culminate at <b>${liveCurrentSupercard(c)}</b>.</p><button class="btn live-primary" onclick="gauntletLiveCalendar()">BEGIN THE MONTH</button></section>`)}
